...
class VncCanvas extends Canvas
  ...
  public void paintFrameBuffer(Graphics g) {
    g.drawImage(memImage, 0, 0, rfb.framebufferWidth, rfb.framebufferHeight, null);
  }
  ...
  void resizeDesktopFrame() {
    ...
    viewer.desktopScrollPane.setSize(screenSize);

    // make the frame larger than the screen:
    Dimension frameSize = new Dimension(screenSize);
    Insets insets = viewer.vncFrame.getInsets();
    frameSize.height += insets.top + insets.bottom;
    frameSize.width  += insets.left + insets.right;
    viewer.vncFrame.setSize(frameSize);

    // and move it to the "North-West" such that the scrollPane exactly fits
    // the screen
    viewer.vncFrame.setLocation(-insets.left, -insets.top);


    // if (needToResizeFrame) {
//       viewer.desktopScrollPane.setSize(scrollPaneSize); // Hack for Epoc Java
//       viewer.vncFrame.setSize(640,240);     
//     }
